...
- 规划提交范围、隐私、二进制、大文件、忽略规则和远端同名分支已通过独立只读审计，当前等待显式 commit/push
- 除 GitHub CLI 外，本轮未安装 OCR、TeX、模型、字体或其他项目依赖

## 当前长期执行的首批动作

1. 显式提交并普通 push 规划包，创建规划草稿 PR；
2. 更新发布审计和`RUN_STATE.json`；
3. 从已确认包含规划包的基线创建/恢复`agent/m0-m5-local-v1`长期集成分支并维护草稿 PR；
4. 执行 M0 入场安全检查；
5. 只在 M0 门禁完成后进入 M1。
