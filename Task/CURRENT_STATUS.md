...
更新时间：2026-07-11 20:36 +08:00

## 长期执行状态

- 计划版本：1.0.1
- 当前阶段：M0 入场审计完成，M0 尚未验收
- 当前切片：`M0-S3-C durable segment ledger`本地冻结门已通过，正在形成并发布内部检查点；下一切片为 S3-D operation lease/job staging/tree evidence，S3 与 M0 门禁均未通过
...
- 生产静态 inventory 覆盖`app/`和`scripts/`的 47 个源文件、451 个副作用入口和 19 个分片；`UNKNOWN=0`，但 451 项仍全部`UNMIGRATED_BLOCKED`
- Policy V7 digest 为`8df50ded...e4d88`；生产 boundary 固定根且`writer_available=false`，`m0_exit_allowed=false`
- S3-C 定向回归 RUN-016 为`427 passed, 1 deselected`；全量回归 RUN-017 为`515 passed, 1 deselected`，JUnit failure/error/skip 均为 0；唯一 deselected 仍是 WinError 1314 的实际 symlink 权限门禁
- RUN-017 保护树前后均 53,260 条、运行时变更 0、句柄围栏 53,260 个；活动 SQLite 前后 SHA-256 相同
- Scanner V7 使用`UTF8_LF_V1`规范化并加入 durable-ledger gateway、named mutex、source-handle rename 与固定 ABI capability 分类；根`.gitattributes`继续固定源码、测试和 Task 证据为 LF
...
- M0-S3-C 本地冻结证据为`RUN-20260711-M0-S3C-WRITER-016-FINAL`与`RUN-20260711-M0-S3C-FULL-017-FINAL`；实现、inventory、ADR 与报告尚待显式提交、普通 push 和远端核验
...
1. M0-S3-C 不可变 audit segment ledger、key revision、启动全链验证、source-handle no-replace publish 与 V7 inventory 已通过本地冻结门，正在形成发布检查点；
2. M0-S3-D—H 建立 operation lease、job staging、实际 tree evidence、pair publish、Copy、quarantine 和受控外部进程；当前仍保持生产 writer 断开；
...
- 固定生产 boundary、Policy V7、Test-only 句柄 writer 和 audit ledger 已通过，但生产 writer 仍断开；451 个入口尚未迁移，在完整 operation/pair/Copy/数据库门禁完成前不得处理真实资料。
- S2 `PairEvidence`仍是调用者声明；S3-C 只完成单文件 source-handle publish 与 audit ledger，S3-D/E 仍须从实际 handles 重算 manifest、源树、条目数和拓扑并绑定业务 mutation。
