...
- 收尾时曾把 91 个被替代目录、8,324 个文件、1,145,431,360 bytes 可恢复地迁到
  `C:\RC2R`；经用户确认后，又对该目录及
  `${LOCALAPPDATA}\Codex\workspace-relief\yanoutrageous-Test` 做了依赖、Git 工作树、
  活动进程和 Reparse 检查，并在隔离改名后、永久删除后各运行一次正式 RC2 校验，
  两次均为 `PASS`。最终永久删除 96,303 个文件、13,285,568,584 logical bytes，
  C 盘实际增加 6,707,093,504 bytes（约 6.25 GiB）可用空间；两个旧恢复根均已不存在，
  不可恢复。RC2、最终 fresh-user 证据、最终门禁和当前 M2/M3/M4 状态均保留在 E 盘，
  当前项目及另一个已识别项目工作树均未改变。
...
下面的 RC1 及更早章节保留为历史记录；其中“可恢复迁到 C 盘”的描述仅表示当时的
阶段动作，相关 C 盘恢复副本已于 2026-07-26 经确认后永久删除。若与本节冲突，以
RC2 本节和对应合同为准。
