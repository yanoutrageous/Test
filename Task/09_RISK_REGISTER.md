...
| R-006 | NEW9 无文本层，无法恢复精确字体/语义 | H | H | S5 已将91页NEW9登记为独立视觉/模板真值并保留实测尺寸；仍不能从轮廓恢复字体/语义 | 分离视觉/内容真值；M1整页回归、字体清单和等效声明 | M0/M1 | Mitigating |
| R-007 | 多种页面尺寸被误当成一个模板 | H | H | 176×250、182×257、184×260、A4、A3 共存；S5已冻结12个模板族 | 版本化模板族和exact-reference页面合同，M1逐族真实金标 | M0/M1 | Mitigating |
| R-008 | MathType OLE、WMF、Corel 轮廓兼容失败 | H | H | 多份 DOCX 含大量 OLE/WMF，答题卡无文本层 | 原件保留、规范化派生、代表压力样本、降级轨 | M1 | Open |
| R-009 | 字体缺失、字宽漂移或商业许可不允许分发 | H | H | 方正、汉仪、宋体等混用；S5登记许可状态并冻结零静默替换阈值 | FontManifest精确revision、哈希/替代检测、许可门禁、客户自备说明；M1/M5真实验证 | M0/M1/M5 | Mitigating |
| R-010 | OCR/裁切/跨页结果看似成功但内容错误 | H | H | 自动识别天然不确定 | 候选状态、原貌对照、人工复核、低置信失败路径 | M1 | Open |
| R-011 | 预计难度与实测 P 含义相反 | M | H | S5 QuestionIR已分离“越高越难”的design difficulty与“越高越易”的observed P，并用合成响应精确复算 | 保持字段、来源和审核分离；M1真实数据仍须复核 | M0/M1 | Mitigating |
| R-012 | 没有逐题真实用时却伪造时间约束 | H | M | S5合成夹具只有整卷时长，合同强制observed item time为空 | expected time人审；逐题实测缺失即留空；M1/M2禁止推导伪值 | M1/M2 | Mitigating |
| R-013 | 蓝图无解时系统静默放宽硬约束 | M | H | 自动求解常见错误行为 | 硬/软分离、不可行核心、属性测试、UI 缺口报告 | M2 | Open |
| R-014 | 五类文档题号/分值/答案漂移 | M | Critical | S5 PaperIR已规定从同一PaperRevision和精确题目/模板revision投影五类角色；M2尚未生成真实bundle | 单一PaperRevision、staging交叉核对、原子bundle和泄漏扫描 | M2 | Mitigating |
| R-015 | 学生卷泄漏答案/隐藏元数据 | M | Critical | 文本层、附件、书签、alt text 可泄漏 | 内容投影和多层泄漏扫描、真实 PDF 检查 | M2/M5 | Open |
| R-016 | SVG/TikZ 视觉正确但数学语义错误 | H | H | S5冻结几何/函数/统计FigureIR和语义约束；生成器与真实人工批准尚未实现 | M3数据/约束检查、人工批准、原图回退 | M3 | Mitigating |
| R-017 | 恶意/非法 SVG 或 TeX 执行主动内容 | M | Critical | S5已把已知非法SVG登记为failure真值，但尚未实现生产安全解析器 | M3禁止外链/脚本/foreignObject/shell escape并对登记失败样本验收 | M0/M3 | Open |
...
| R-021 | 视觉基线被自动更新以掩盖退化 | M | H | S5 registry要求revision、确认引用、理由和previous aggregate hash，旧基线不可覆盖 | 每次更新独立D2审计；像素指标只作诊断，不能覆盖锚点/字体/人工叠图失败 | M3 | Mitigating |
...
| R-025 | 批量渲染/备份耗尽磁盘并诱发危险清理 | M | Critical | 参考资产约2.6 GiB，派生可能倍增；S5全量热峰值约127 MiB | 日常core/阶段full分层；17个冷归档只在生成/冻结校验；仅保留RUN-123热证据；旧目录经校验后可恢复迁移到C；S5后E盘约62.64 GiB可用 | All | Mitigating |
| R-026 | CDN/遥测接触成绩和题库 | H | Critical | 现有 HTML 引用远程脚本 | 静态依赖本地化、CSP、断网验收、网络审计 | M0/M3/M5 | Open |
| R-027 | PII 进入日志、embedding、Git 或客户包 | H | Critical | XLSX含姓名、学号、IP、QQ；S5 tracked registry无路径/文件名，3个internal和1个unknown保持受限 | restricted Copy、匿名化、扫描、禁止列表；M5客户包再次阻断验证 | All | Mitigating |
| R-028 | 原卷、解析、字体许可阻止客户分发 | M | H | S5明确4个local-use-only、2个unverified；本地使用不等于可再分发 | 许可清单、客户自备、copy-on-use和M5发布阻断 | M0/M5 | Mitigating |
...
| R-041 | 固定盘符、旧主机锁或绝对路径使迁移后授权错误 | H | Critical | portable root已在E位置通过；S5 tracked合同/registry只含相对路径或逻辑ID，本机gold映射继续Git忽略 | marker+受信任模块确定唯一根；跨机重建本机映射并复核源hash、数据库和安全基线 | M0/All | Mitigating |
